...
# Practice set info ---- 
practice.begin("A2", learner="[Ambrose Payne]", email="apayne5@uw.edu")
...
#    The Goals of Count Love are to post about and update the amount of protests
#    f
#
#
#
...
protests <- read.csv("data.csv", stringsAsFactors = FALSE)
View(protests)

#     *BEST PRACTICE:* Use View() to open and examine the dataset. Some key questions to ask:  
#        1. What information is available? 
#        The information available to us is basic info about each protest and where
#        they are cited.
#        2. Are there missing values (NA, which means "Not Available.") or odd values? 
#        There are N/A's in terms of the amount of people that showed up to the demonstrations
#
#        3. What are the column names (sometimes called variables or features)?
#        The comlumn names are: date, Location, Atendees, Event Tags (what is being protested), and cites
...
num_features <- sum(!is.na(protests))
...
median_attendees <- median(max_attendees, na.rm = TRUE)

# 2f: What is the difference between the mean and median number of attendees? (Variable: `mean_median_diff`)

mean_med_difference <- mean_attendees - median_attendees 
...
#The difference between the two show that the data might not be organized
#from largest to smallest and that the data shows the events at random despite
#order of attendance. 
...
num_locations <- length(unique(num_locations))
...
num_in_wa <- str_detect(protests$Location, "WA")
sum(num_in_wa, na.rm = TRUE)

# 3d: What proportion of protests occurred in Washington? (Variable: `prop_in_wa`)

prop_in_wa = 1375/ 38097

#                                         Note 09.
#     *R3a: REFLECTION:* Does the number of protests in Washington surprise you?
#     Why or why not?

# The number of protests in Washington doesn't surprise me, partly because we 
# represent a proportion of total protests over time according to this data. Considering we are a fairly
# activist state, we represent a good proportion of activity not depending on the issue.
...
count_in_location <- function (location) {
  x <- sum(str_detect(protests$Location, location), na.rm = TRUE)
}%>%{print(x +"protests happened here")
